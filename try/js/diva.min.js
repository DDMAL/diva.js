...
0,c.mapSize,c.mapSize),b("#diva-map-viewbox").show(),m());r();q(f);A();"function"===typeof h&&h.call(h)};a.src=g;if(a.complete||void 0===a.complete)a.src="data:image/gif;base64,R0lGODlhAQABAIAAAAAAAP///ywAAAAAAQABAAACAUwAOw==",a.src=g},J=function(){var a=h[g],c=a.current,a=a.transform?a.transform(c):c;b("#diva-canvas-value").html(a)},x=function(){b("#diva-canvas-slider").val(h[g].current)},y=function(a){a=c.zoomWidthRatio*Math.pow(2,a);return c.proxyURL?c.proxyURL+"?f="+c.filename+"&w="+a:c.iipServerURL+
"?FIF="+(c.imageDir+"/")+c.filename+"&WID="+a+"&CVT=JPEG"},z=function(){(0<h.zoom.current||c.mobileWebkit)&&b(c.selector+"throbber").addClass("canvas-throbber").show()},A=function(){b(c.selector+"throbber").removeClass("canvas-throbber").hide()},I=function(){var a={},b=!1,e=c.localStoragePrefix+c.filename,f;for(f in h)h[f].previous!==h[f].initial&&(a[f]=h[f].previous,b=!0);b?(c.pluginIcon.addClass("new"),localStorage.setObject(e,a)):(c.pluginIcon.removeClass("new"),localStorage.removeItem(e))},G=
function(a,b){c.zoomLevel=a;var g=y(a);S(g,function(){e.scaleFactor=e.size/f.size;m();I()})},K=function(a){switch(a.keyCode){case 38:return b("#diva-canvas-wrapper").scrollTop(document.getElementById("diva-canvas-wrapper").scrollTop-c.arrowScrollAmount),!1;case 40:return b("#diva-canvas-wrapper").scrollTop(document.getElementById("diva-canvas-wrapper").scrollTop+c.arrowScrollAmount),!1;case 37:return b("#diva-canvas-wrapper").scrollLeft(document.getElementById("diva-canvas-wrapper").scrollLeft-c.arrowScrollAmount),
!1;case 39:return b("#diva-canvas-wrapper").scrollLeft(document.getElementById("diva-canvas-wrapper").scrollLeft+c.arrowScrollAmount),!1}};return{init:function(a,l){if(!window.HTMLCanvasElement)return!1;b.extend(c,k,a.canvasPlugin);c.inCanvas=!1;c.iipServerURL=a.iipServerURL;c.imageDir=a.imageDir;c.selector=a.selector;c.mobileWebkit=a.mobileWebkit;c.arrowScrollAmount=a.arrowScrollAmount;h={contrast:{initial:1,min:c.contrastMin,max:c.contrastMax,step:c.contrastStep,transform:function(a){return a.toFixed(2)},
...
max:c.rgbMax,step:1,title:"Adjust the blue channel"}};var v=function(){var a,b,c;for(c in h)b=h[c],a=b.initial,b.current=a,b.previous=a};v();if(b("#diva-canvas-backdrop").length)return!0;var n=[],s,y,N;for(N in c.buttons)s=c.buttons[N],y=h[s].title,s='<div class="'+s+'" title="'+y+'"></div>',n.push(s);n='<div id="diva-canvas-backdrop">'+('<div id="diva-canvas-tools"><div id="diva-canvas-toolbar"><div id="diva-canvas-close" title="Return to the document viewer"></div><div id="diva-canvas-minimise" title="Minimise the toolbar"></div><span id="diva-canvas-info">Test</span></div><div id="diva-canvas-toolwindow"><div id="diva-map-viewbox"></div><canvas id="diva-canvas-minimap"></canvas><div id="diva-canvas-buttons">'+
n.join("")+'</div><div id="diva-canvas-pane"><p id="diva-canvas-tooltip"><span id="diva-canvas-mode">contrast</span>: <span id="diva-canvas-value">0</span> <span id="diva-canvas-reset" class="link">(Reset)</span></p><input type="range" id="diva-canvas-slider"></input></div><br /><div class="action-buttons"><a href="#" id="diva-canvas-reset-all">Reset all</a><a href="#" id="diva-canvas-apply">Apply</a></div></div></div>')+'<div id="diva-canvas-wrapper"><canvas id="diva-canvas"></canvas></div></div>';
b("body").append(n);c.mapSize=b("#diva-canvas-minimap").width();b("#diva-canvas-buttons div").click(function(){b("#diva-canvas-buttons .clicked").removeClass("clicked");H(b(this).attr("class"))});var H=function(a){g=a;a=h[g];b("#diva-canvas-buttons ."+g).addClass("clicked");b("#diva-canvas-mode").text(g);var c=a.current,e=a.transform?a.transform(c):c,f=document.getElementById("diva-canvas-slider");f.min=a.min;f.max=a.max;f.step=a.step;b("#diva-canvas-slider").val(c);b("#diva-canvas-value").html(e)};
H("contrast");b("#diva-canvas-slider").on("input",function(a){h[g].current=parseFloat(this.value);J();r()});b("#diva-canvas-reset-all").click(function(){for(var a in h)h[a].current=h[a].initial;J();x();r()});b("#diva-canvas-reset").click(function(){h[g].current=h[g].initial;J();x();r()});b("#diva-canvas-apply").click(function(){u()&&(z(),setTimeout(function(){h.zoom.current!==h.zoom.previous?G(h.zoom.current):(q(),A(),I())},c.throbberTimeout))});b("#diva-canvas-close").click(function(){b("body").removeClass("overflow-hidden");
f.context.clearRect(0,0,f.size,f.size);e.context.clearRect(0,0,e.size,e.size);b("#diva-canvas-wrapper").scrollTop(0).scrollLeft(0);b("#diva-canvas-backdrop").hide();b("#diva-map-viewbox").hide();A();l.enableScrollable();b(document).off("keydown",K);v();J();x();b("#diva-canvas-buttons .clicked").removeClass("clicked");H("contrast");diva.Events.publish("CanvasViewDidHide")});b("#diva-canvas-minimise").click(function(){b("#diva-canvas-toolwindow").slideToggle("fast")});b(window).resize(function(){c.viewport=
{height:window.innerHeight-a.scrollbarWidth,width:window.innerWidth-a.scrollbarWidth};c.inCanvas&&m()});b("#diva-canvas-wrapper").scroll(function(){c.inCanvas&&m()});b("#diva-canvas-minimap, #diva-map-viewbox").mouseup(function(a){var f=b("#diva-canvas-minimap").offset(),g=(a.pageX-f.left)/e.scaleFactor;a=(a.pageY-f.top)/e.scaleFactor;b("#diva-canvas-wrapper").scrollTop(a-c.viewport.height/2);b("#diva-canvas-wrapper").scrollLeft(g-c.viewport.width/2)});b("#diva-canvas").mousedown(function(){b(this).addClass("grabbing")}).mouseup(function(){b(this).removeClass("grabbing")});
