                ...
                case Dict.get flowId model.flows of
                    ...
                    Just flow ->
                        case flow.phase of
                            ...
                            ProbingFresh ->
                                retryOrFail flowId (probeError flow result.status) model
                                    |> addEffects [ RemoveToken flowId (tokenStorageKey flow.association) ]
    ...
    case ( Dict.get flowId model.flows, value ) of
        ( Just flow, Just stored ) ->
            case Decode.decodeValue cachedTokenDecoder stored of
                Ok cached ->
                    if Maybe.map ((<) now) cached.expiresAt |> Maybe.withDefault True then
                        let
...
probeError : Flow -> Int -> String
probeError flow status =
    case flow.association.probe.errorHeading of
        Just _ ->
